! add a function used in akbrdf -- integr
function integr(xx)
    implicit none
    real(8),intent(in) :: xx
    real(8) :: integr

    integr = (1.d0 - exp(-xx))/xx
...
!
    ...
    real(8) :: pi12, pi, eps4, cth10, rrls, sth10, rtp
...
!
    ...
    real(8) :: tt3(10), stt3(10), ctt3(10), tt2(10), stt2(10), ctt2(10)
    ...
    real(8) :: pi, pi1, eps, bd1, bd2, rrls, rtp, thi
    integer :: i, i1, i2, j
...
!
    ...
    real(8) :: alpd, bam, bc1, bc1d, bc1hs, bcsp, bs1, catmp, cth1s, &
        cths, easte, easte2, easte4, eps, eps3, gammd, gf, gg, gg1, glthp, &
        gma, pi, sct, sth1s, th1s, thp, ths, ulg, ulg1, x, x2, xx1, xx2, y4, sths
!
...
!
    xx1   = (1.d0 - easte)/gg1
    xx2   = (1.d0 - easte2)/(gg1*.5d0 + bam)-(1.d0 - easte4)/(gg1 + bam)
    bc1d  = xx1*gammd
    bc1hs = xx2*(gammd + gf)
    bcsp  = xx1*gf
    bc1   = bc1d + bcsp + bc1hs
...
!
    ...
    implicit none
    real(8), external :: integr
    real(8) :: ks, ko, m, m11, m12, m21, m22
...
!
    ...
    real(8) :: a, att, b, bf, c, d, det, ek, em, ems, ep, epso, epss, &
        gg, gm, gp, h1, h2, rdsoil, rplants, rtp, sb, sf, sig, tanto, &
        tants, tsun, tview, ub, uf
!
...
!
    ks    = g1*ul/cth1
    ko    = g*ul/cth
    gg    = (1.289d0*difmy - 1.816d0*difsig)*(cthm**2 -.33333333333d0)+.31823d0
...
!
    ...
    real(8) :: rrl, ttl, ul, sl, clmp, clmp1, bi, bd, bqint
    real(8) :: ee, thm, sthm, cthm
    real(8) :: glth, th, eps, u1, u2, u3, u4, x, x1, x2
!
    common /aaa/ rrl, ttl, ul, sl, clmp, clmp1, bi, bd, bqint
    common /ladak/ ee, thm, sthm, cthm
!
...
!
    ...
    real(8) :: gf, ag, bg, ca, cg, sa2, x2, xy, y, yy
!
...
!
    ...
    common /soildata/phis1, phis2, phis3, phis4, rsl1, rsl2,          &
                rsl3, rsl4, th2, rsl, rsoil, rr1soil, rrsoil
!
    rsl = rsl1*phis1(jl)+rsl2*phis2(jl)+rsl3*phis3(jl) + rsl4*phis4(jl)
!
    return
end
!
...
!
    ...
    real(8) :: cfa, cfb, ee, eps5, fa, fb, gr0, gr1, gt0, gt1, pi, pi12, &
        pi13, pi14, pi4, pp, sfa, sfb, sg, sg1, sgmr, sgmt, x, x1, x2, y, y1
    integer :: i1, ii, j, l2, l4
!
...
48  continue
    ...
    gr1 = sgmr*pi12
    gt1 = sgmt*pi12
    gr = gr0 - .0102d0 + (1.742d0*difmy - .4557d0*difsig)*(gr1 - gr0)
    gt  = gt0 + .00653d0 + (.2693d0*difmy + 5.821d0*difsig)*(gt1 - gt0)
...
!
    !implicit real(8) (a-h, o-z)
    implicit none
    real(8) :: vai, refl, tran, teta, ref, tau, alfa, beta, delta, r, ra, &
        s1, s2, s3, t, t1, t2, ta, va, vb, x1, x2, x3, x4, x5, x6
!
    real(8) :: nn, k, inex
...
!   ******************************************************************
!   ALLEN et al.,  1969,  Interaction of isotropic ligth with a compac
...
!   ******************************************************************
    ...
    s1    = ra*(va*vb**(vai - 1.d0) - va**(-1.d0)*vb**(-(vai - 1.d0))) +  &
...
!
    ! implicit real(8) (a-h, o-z)
    implicit none
    real(8) :: k, teta, ref, tau, dr, eps, pi12, a, b, b1, b2, ds, r2, rm, &
        rp, tp, tp1, tp2, tp3, tp4, tp5, ts, xxx
!
    common /tauin/ teta, ref
    common /tauout/ tau
!
...
!
    !real(8) ke, kab, kw
    real(8) :: ref(200), ke(200), kab(200), kw(200)
    common /dat/ ref, ke, kab, kw
!
    real(8) :: phis1(200), phis2(200), phis3(200), phis4(200), rsl1, &
        rsl2, rsl3, rsl4, th2, rsl, rsoil, rr1soil, rrsoil
    common /soildata/ phis1, phis2, phis3, phis4, rsl1, rsl2,         &
        rsl3, rsl4, th2, rsl, rsoil, rr1soil, rrsoil
    integer :: i
!
...
!
    ...
    integer :: np, mu
    integer :: k, j
    real(8) :: eei, thmi, uli, sli, rsl1i, wlmoy, rnci, cabi, cwi, vaii, pir
    real(8) :: mu1, mu2, fi
...
!
    ...
    common /dat/ refr, ke, kab, kw
!
    real(8) :: phis1(200), phis2(200), phis3(200), phis4(200), rsl1, rsl2,         &
        rsl3, rsl4, th2, rsl, rsoil, rr1soil, rrsoil
    common /soildata/ phis1, phis2, phis3, phis4, rsl1, rsl2,         &
        rsl3, rsl4, th2, rsl, rsoil, rr1soil, rrsoil
!
    ...
    real(8) :: clx, clz, cth10, dr, eln, eps, eps4, pi, pi12, rlambda, rrls, &
        sth10, tgt, tgt1, xx
!
!
...
!           print *, 'msrm93'
!
    ! integr(xx) = (1.d0 - exp(-xx))/xx !! HyS function integr
    jg = 1
!   if (lf .eq. 1) then
!
    ee    = eei
...
!    & (eei, thmi, uli, sli, rsl1i, wlmoy, rnci, cabi, cwi, vaii, albbrd
!
...
!
    ...
    implicit none
!
!   real(8) eei, thmi, uli, sli, rsl1i, wlmoy, rnci, cabi, cwi, vaii, alb
    real(8) :: albbrdf
    save /count/, /soildata/, /aaa/, /ggg/, /ladak/
!
    real(8) :: uu(48), aa(48)
