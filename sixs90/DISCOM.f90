subroutine discom(idatmp,iaer,iaer_prof,xmus,xmuv,phi,taer55,                   &
                  ...
                  roatm_fi,nfi,nfilut,filut,roluts,rolutsq,rolutsu)
...
! - to vary the number of quadratures
    ...
    real(8) :: ftray,xlm1(-mu:mu,np),xlm2(-mu:mu,np)
    real(8) :: roatm_fi(3,20,nfi),rorayl_fi(nfi),romix_fi(nfi)

    real(8) :: rolut(mu,61),roluts(20,mu,61)
    real(8) :: rolutq(mu,61),rolutsq(20,mu,61)
    real(8) :: rolutu(mu,61),rolutsu(20,mu,61)
    real(8) :: filut(mu,61)
    integer :: nfilut(mu)

    real(8) :: xmus,xmuv,phi
    real(8) :: taer55,taer55p,palt,phirad,ext,ome,gasym,phase,roatm
    real(8) :: dtdir,dtdif,utdir,utdif,sphal,wldis,trayl,traypl,s
    real(8) :: wlinf,wlsup,wl,tray,trayp,taer

    real(8) :: rqatm,ruatm,rurayl,ruaero,rumix,qhase,uhase
    real(8) :: taerp,piza,tamoy,tamoyp,pizmoy,rorayl,rqrayl,rqmix,rqaero
    real(8) :: roaero,romix,ddirtt,ddiftt,udirtt,udiftt,sphalbt,ddirtr
    real(8) :: ddiftr,udirtr,udiftr,sphalbr,ddirta,ddifta,udirta,udifta
    real(8) :: sphalba,coeff
...
!   discrete values,so we can interpolate at any wavelength
	...
	! HyS, WuTeng
        ...
        !if ((l.lt.20).and.(wldis(l).lt.wlinf).and.(wldis(l+1).lt.wlinf)) goto 50
        !if ((l.gt.1).and.(wldis(l).gt.wlsup).and.(wldis(l-1).gt.wlsup)) goto 50
...
!     Legendre polynomials.
        ...
        call atmref(iaer,iaer_prof,tamoy,taer,tray,pizmoy,piza,tamoyp,               &
                    taerp,trayp,palt,phi,xmus,xmuv,phirad,nt,mu,np,rm,gb,rp,rorayl,  &
                    roaero,romix,rqrayl,rqaero,rqmix,rurayl,ruaero,rumix,ipol,       &
                    xlm1,xlm2,rorayl_fi,romix_fi,nfi,                                &
...
!     first time for rayleigh ,next total (rayleigh+aerosols)
       call scatra(iaer_prof,tamoy,tamoyp,tray,trayp,pizmoy,                    &
                   palt,nt,mu,rm,gb,                                            &
                   xmus,xmuv,ddirtt,ddiftt,udirtt,udiftt,sphalbt,ddirtr,ddiftr, &
                   udirtr,udiftr,sphalbr,ddirta,ddifta,udirta,udifta,sphalba)
        dtdir(1,l)=ddirtr
        dtdif(1,l)=ddiftr
        dtdir(2,l)=ddirtt
        dtdif(2,l)=ddiftt
