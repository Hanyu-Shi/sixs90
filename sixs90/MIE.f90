...
! - to vary the number of quadratures
    real(8) :: nr,ext(20,4),sca(20,4),np(4),p1(20,4,nqmax_p)
    real(8) :: q1(20,4,nqmax_p),u1(20,4,nqmax_p)
    real(8) :: p11(nqmax_p),q11(nqmax_p),u11(nqmax_p)
    real(8) :: pi,r,rmind,rmaxd,r0,alpha,dr,xndpr2,Qext,Qsca
    real(8) :: rlogpas
    real(8) :: ex(4,20),sc(4,20),asy(4,20),wldis(20)
    real(8) :: rmax,rmin,rn,ri,x1,x2,x3,rsunph,nrsunph
    real(8) :: asy_n,asy_d,cij
...
! call for the size distribution nr. For our computation, we need dn/dr
! all functions except for sun-photometer inputs, for which we need dV/d
...
! The Mie's calculations have to be called several times (min=2, max=10
! each type of particle): at wavelengths bounding the range of the selec
! wavelengths,and at 0.550 microns to normalized the extinction coeffici
! (if it's not in the selected range of wavelengths).

398     nr=nr*3./(pi*r*r*r*r*4.)
399     continue
        xndpr2=nr*dr*pi*(r**2.D+00)
! relatif number of particle for each type of particle (has to be equal
        np(i)=np(i)+nr*dr

        do l=1,20
            alpha=2.D+00*pi*r/wldis(l)
            call EXSCPHASE(alpha,rn(l,i),ri(l,i),Qext,Qsca,p11,q11,u11,ipol,cgaus_S,pdgs_S)
...
! extinction efficiency factors (usually written Qsca and Qext) and it a
! computes the scattering intensity efficiency
subroutine EXSCPHASE(X,nr,ni,Qext,Qsca,p11,q11,u11,ipol,cgaus_S,pdgs_S)
    use paramdef
    implicit none
    integer :: ipol
    integer,parameter::nser=10000000
    real(8) :: Ren,Imn,X,Y,Up,XnumRDnY,XnumIDnY
    real(8) :: XdenDnY,coxj,Qsca,Qext,xJonH,XdenGNX
    real(8) :: Xnum1An,Xnum2An,XdenAn,Xden1An,Xden2An,RAnb,IAnb
    real(8) :: Xnum1Bn,Xnum2Bn,XdenBn,Xden1Bn,Xden2Bn,RBnb,IBnb
    real(8) :: xmud,xpond,RS1,RS2,IS1,IS2,co_n,test
    real(8) :: xj(0:nser),xy(-1:nser),Rn(0:nser)
    real(8) :: IDnY(0:nser),RDnX(0:nser),RDnY(0:nser)
    real(8) :: IGnX(0:nser),RGnX(0:nser)
    real(8) :: RAn(0:nser),IAn(0:nser),RBn(0:nser),IBn(0:nser)
    real(8) :: TAUn(0:nser),PIn(0:nser)
    real(8) :: nr,ni
...
! In certain this value of N above leads to a value of Up~1 and big prob
...
!     becomes monotonic. We start at a order greater than this transitio
...
!     downward recursion    - real(8) and imaginary parts
    RDnY(mu)=0.D+00
    IDnY(mu)=0.D+00
    RDnX(mu)=0.D+00
    do k=mu,1,-1
...
! --- Computation of Gn(alpha), real(8)  and Imaginary part
        XdenGNX=(RGnX(k-1)-k/X)**2.D+00+IGnX(k-1)*IGnX(k-1)
        RGnX(k)=(k/X-RGnX(k-1))/XdenGNX-k/X
        IGnX(k)=IGnX(k-1)/XdenGNX

! --- Computation of An(alpha) and Bn(alpha), real(8) and Imaginary part
